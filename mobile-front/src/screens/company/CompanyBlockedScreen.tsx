...
export const CompanyBlockedScreen: React.FC = () => {
    ...
    switch (status) {
      case AccountStatus.PENDING:
        return {
          ...
          messageColor: "#92400E",
          logoutBg: "#D97706",
          titleKey: "company.status.pendingTitle",
          messageKey: "company.status.pendingMessage",
        };
      case AccountStatus.REJECTED:
        return {
          ...
          messageColor: "#991B1B",
          logoutBg: "#DC2626",
          titleKey: "company.status.rejectedTitle",
          messageKey: "company.status.rejectedMessage",
        };
      case AccountStatus.SUSPENDED:
        return {
          ...
          messageColor: "#5B21B6",
          logoutBg: "#7C3AED",
          titleKey: "company.status.suspendedTitle",
          messageKey: "company.status.suspendedMessage",
        };
      case AccountStatus.DELETED:
        return {
          ...
          messageColor: "#334155",
          logoutBg: "#334155",
          titleKey: "company.status.deletedTitle",
          messageKey: "company.status.deletedMessage",
        };
      default:
        return {
          accent: ACCENT,
          ...
          messageColor: "#6D28D9",
          logoutBg: "#7C3AED",
          titleKey: "company.status.unknownTitle",
          messageKey: "company.status.unknownMessage",
        };
    }
  }, [status]);
  ...
  const handleResubmit = async () => {
    if (pendingDocs.length === 0) {
      setDocError(t("company.status.resubmitDocumentsRequired"));
      return;
    }
    setDocError(null);
    setSubmitting(true);
    try {
      ...
      if (!session?.user?.id) {
        throw new Error(t("company.status.resubmitNotSignedIn"));
      }
      ...
      setNotice({
        visible: true,
        title: t("company.status.resubmitSuccessTitle"),
        message: t("company.status.resubmitSuccessMessage"),
      });
    } catch (e: unknown) {
      const msg =
      ...
      setNotice({
        ...
        message:
          msg ||
          (e instanceof Error ? e.message : t("company.status.resubmitError")),
      });
    } finally {
      setSubmitting(false);
    }
      ...
      >
        ...
        >
            ...
            {status === AccountStatus.REJECTED ? (
              <View style={styles.adminFeedback}>
                <View style={styles.adminFeedbackHeader}>
                  ...
                  >
                    {t("company.status.adminFeedbackTitle")}
                  </Text>
                </View>
                {loadingComment ? (
          ...
          {status === AccountStatus.REJECTED ? (
            <View style={styles.resubmitCard}>
              <Text style={[styles.resubmitHeading, isRTL && styles.rtlText]}>
                {t("company.status.resubmitHeading")}
              </Text>
              <Text style={[styles.resubmitHint, isRTL && styles.rtlText]}>
                {t("company.status.resubmitHint")}
              </Text>

              <Text style={[styles.fieldLabel, isRTL && styles.rtlText]}>
                {t("company.status.ownerCommentLabel")}
              </Text>
              <TextInput
                style={[
                  styles.commentInput,
                ...
                placeholder={t("company.status.ownerCommentPlaceholder")}
              ...
              >
                {t("company.status.resubmitDocumentsTitle")}
              </Text>
          ...
          >
            ...
            >
              {t("company.status.checkStatusAgain")}
            </Text>
          </TouchableOpacity>
