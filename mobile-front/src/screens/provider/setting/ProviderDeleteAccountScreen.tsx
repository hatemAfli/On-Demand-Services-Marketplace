...
type Nav = NativeStackNavigationProp<
  ...
  "ProviderDeleteAccount"
>;

export const ProviderDeleteAccountScreen: React.FC = () => {
  ...
  const onConfirmDelete = async () => {
    ...
    setLoading(true);
    try {
      await api.softDeleteProviderAccount({ password });
      setNoticeModal({
        visible: true,
        title: t("provider.settings.deleteAccountSuccessTitle"),
        message: t("provider.settings.deleteAccountSuccessMessage"),
        isSuccess: true,
      });
    } catch (e: any) {
      ...
      if (status === 401) {
        setNoticeModal({
          ...
          message: t("provider.settings.deleteAccountWrongPassword"),
          isSuccess: false,
        });
      } else {
        setNoticeModal({
          visible: true,
          title: t("common.error"),
          message: message || t("provider.settings.deleteAccountError"),
          isSuccess: false,
        });
      }
    } finally {
          ...
          <View style={styles.card}>
            <View style={styles.headerRow}>
              ...
              <View style={styles.headerTextWrap}>
                <Text style={[styles.title, isRTL && styles.rtlText]}>
                  {t("provider.settings.deleteAccountTitle")}
                </Text>
                <Text style={[styles.subtitle, isRTL && styles.rtlText]}>
                  {t("provider.settings.deleteAccountSubtitle")}
                </Text>
              </View>
            </View>
            ...
            <View style={styles.warningBox}>
              <Text style={[styles.warningText, isRTL && styles.rtlText]}>
                {t("provider.settings.deleteAccountWarning")}
              </Text>
            </View>
              ...
              {loading ? (
                ...
                <View style={styles.deleteButtonContent}>
                  ...
                  <Text style={styles.deleteButtonText}>
                    {t("provider.settings.deleteAccountButton")}
                  </Text>
                </View>
              )}
            </TouchableOpacity>
